  ...
  tables: {
    server_user: {
      columns: [
        ...
        ['role_mappings_2', ['stringarray'], 'not unique', 'nullable', 'mutable'],
...
serverTables.forEach((table) => {
  ...
  karyaServerDb_new.tables[table].columns = serverSideServerIdFields.concat(columns).concat(commonFields);
  karyaBoxDb_new.tables[table].columns = boxSideServerIdFields.concat(columns).concat(commonFields);
});

boxTables.forEach((table) => {
  const columns = karyaServerDb_new.tables[table].columns;
  karyaServerDb_new.tables[table].columns = serverSideBoxIdFields.concat(columns).concat(commonFields);
  karyaBoxDb_new.tables[table].columns = boxSideBoxIdFields.concat(columns).concat(commonFields);
  karyaBoxDb_new.tables[table].triggers = [computeIDTrigger(table)];
});
...
karyaBoxDb_new.tables['karya_file'].triggers = [computeIDTrigger('karya_file')];

export { karyaServerDb_new, karyaBoxDb_new };
